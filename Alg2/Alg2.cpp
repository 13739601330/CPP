//***********************************************************************************************************************************************************
//Instruction
//Parameter setting starts at line 36 (after the include and using namespace command).

//Generally, only need to choose N, k, num_test and rounds.   只要改变这几个变量即可
//Then choose suitable value for first_main_prime and number_of_sample_sets. 
//Only increase the value of kappa when you want more accurate result.
//Also one include the directory location of the fftw3.h in the include command too. 
//For example: one can change #include <fftw3.h> to #include </User/username/usr/include/fftw3.h> to let the compiler know the correct path of fftw3.h.

//We have to compile this with primes.h using Makefile. 

//This code runs numerical experiment for Discrete MSFT (DMSFT) algorithm. 
//It is worth to mention that both Alg2 and Alg2_fftw use the same algorithm (DMSFT)
//The mejority difference between this Alg2_fftw and Alg2 is the method we use to generate uniform samples.
//In Alg2_fftw we use fftw3 to generate uniform samples, this means the total number of uniform samples is equal to bandwidth N.
//In Alg2 we only generate those uniform samples which will be used in DMSFT algorithm
//In our numerical experiment, we use Alg2_fftw to deal with the signal which has large sparsity (i.e. 4000)
//We use Alg2 to deal with the low sparsity and high bandwidth signal (e.g. bandwidth = 2^30, sparsity = 50)
//***********************************************************************************************************************************************************
#include <complex>
#include <iostream>
#include <vector>
...
//***********************************************************************************************************************************************************
//Below is the part of code for parameter setting 变量设置
//***********************************************************************************************************************************************************
...
//use this index to track the correct location of sig_sample  追踪sig-sample位置 指针（信噪比）-不可变常的设置
...
//In this function, we try to find the index of the smallest main prime number  最小主要原始数据的指针
...
int index_of_smallest_main_prime(int N, int k) {
	...
	//this vector（矢量） record which prime number shows up previously
	//0 indicate does not show up; otherwise indicate shows up
	vector<int> show_up(prime_num);
	//alpha record the power 幂 of each prime numbers
...
//In this part we try to build a sequence of prime numbers that we will use in our sampling scheme.  采样方案中用的质数（原始？）序列
//Here we follow the principle that we want product of the sequence p_l^(alpha_l) goes to N/S as slow as possible. 计算结果的原则
//More specifically, the scheme we have will be q_j^(alpha_j), q_j^(alpha_j)*p_j1^(beta_j1), q_j^(alpha_j)*p_j1^(beta_j1)*p_j2^(beta_j2)....
//This prime sequence should satisfy q_j^(alpha_j)*p_j1^(beta_j1)*p_j2^(beta_j2)...>=N 条件
//Also we want sum(q_j^(alpha_j), p_j1^(beta_j1), p_j2^(beta_j2)...) to ba as small as possible 条件
//And for each p_j1^(beta_j1), p_j2^(beta_j2)... they should be close to each other as much as possible. 重要条件
...
//利用欧拉乘积公式计算欧拉定理中φ函数的值
//In this function we calculate the value for phi（φ） function in Fermat-Euler 费马-欧拉Theorem 即欧拉定理 by using Euler's product formula 欧拉乘积公式 first, 
//Then return the value of phi function - 1, we will then use this value to calculate the modular inverse（模的逆）.
//Here the input n = q^(alpha)*p_1^(beta_1)*p_2^(beta_2)...*p_j^(beta_j)
//prime is a vector contains q, p_1, p_2, ..., p_j 矢量格式 
...
long long int InverseModP(int a, int phi_n_1, int n)
{
	...
	//When q is a prime number, the Euler's totient function of q^alpha is equal to q^(alpha)-q^(alpha-1) 为啥？
	...
	//equation等式: ab mod n = (a mod n)(b mod n) mod n
	while (temp1 != 0)
	{
		if (temp1 % 2 == 1)   //check whether it's current digit数字 in base 2 is 1
		{
			temp = (temp*y) % n;
		}
		y = (y*y) % n;
...
//In this part we create one row行 for sample scheme matrix 矩阵、模型, it is in the form: 为采样集矩阵创建一个行向量
...
//create struct, struct contains four elements: 创建一个有四个元素的结构体
...
//Generate sample_scheme, which is in the form: 创建sample-scheme
...
//Here m is a number relative to sparsity, N is the bandwidth m与稀疏度有关，N是带宽
void sampling_scheme_recovery_info(vector<vector <int> >& sample_scheme, vector<vector <recovery_info> >& Recovery_Info, int m) {
	...
	for (int i = 0; i < m; ++i) {
		//we will use temp1 to store subvector（子向量） of prime, for each loop, we initialize temp1 first in order to set it as an empty vector.
		vector<int> temp;
...
//generate fftw_plans as a dictionary（？）: map <int, fftw_plan> fftw_plans（？） 构造fftw-plans
//the element of fftw_plans is a pair, for which contains two part:
//int: corresponding to the length of DFT （DFT的长度）
//fftw_plan: store the pointer of fftw_plan with corresponding length 存放相应长度变量的指针
//It is worth to mention that we can use length of sequence as the key is because
//we use pairwise 成对的 prime number to construct 构造 our sampling scheme
...
/*
//exp function 指数函数 is very slow, since in create_sample part, the x in exp(x) always in the interval [-2*pi, 0] x取值范围
//so we generate the value of exp function on equal spaced points 间隔点(the number of points determined by precision 精度we want) on interval [-2*pi, 0]
//then everytime we need to calculate exp(x), we just use linear interpolation to approximate the exp(x) 用线性插值来近似估计指数函数的值
//the precison can be guaranteed because: 1. the exp function in [-2*pi, 0] is smooth; 
//2. we can always get the precision we need by increasing number of pre-calculating equal spaced points. 通过增加间隔点提高精度
...
//create value of G(x_j - x) that needed in convolution part (create_sample function) 为卷积做准备 
...
//the first int is the sample size 样本大小, the second int means the shift of filter function 滤波函数的转换
...
//generate samples  创建采样
//input: sig_sample(equispaced sample from signal信号的等距采样), samples_voctor(store a single needed sample sequence)
//samples: a vector with elements as vector, this will be uesed to store all the samples needed in SFT algorithm 存放稀疏傅里叶变换的所有采样
//conv_G: pre-computed values for periodic Gaussian function G  周期性高斯函数的预算值
//sample_size: size of current sequence
//filter_shift: determine how to shift the filter function 如何进行傅里叶变换
//result: add one vector to samples

//create different equispace samples 平均采样from pre-setting simple testing function 创建不同的平均采样
...
//create signal sample at x. 在x处创建信号采样
...
//generate needed uniform samples only   只生成所需的均匀样本
//the samples generated from this funciton are a subset 子集of {*pi*j/N for j = 0,...,N-1}
//input: 
...
//freq_coeff: vector contains the coefficients of significant frequencies 频率系数

//result: add one vector to sig_samples

//create different equispace samples from pre-setting simple testing function 创建不同均匀采样
...
//calculate Discrete Fourier Transform by using FFTW 利用FFTW计算离散傅里叶变换
//input: vector<fftw_complex*>& samples (vector that cotains all the samples)
//input: map <int, fftw_plan>& fftw_plans, this is the 
//inout: vector<int>& sample_scheme_flat: a vector stores the length of all needed sequence in MSFT   （inout是啥，MSFT是啥？）
...
//define own function to compare two pairs <double number, index(integer)> 比较啥？ 布尔函数
bool mag_complex(pair <complex<double>, int> a, pair <complex<double>, int> b)
{
	return abs(a.first) > abs(b.first);
}

//convert fftw to complex<double> 把FFTW转换
complex<double> fftw_to_std(fftw_complex c){
	return {c[0], c[1]};
}     //布尔函数，也没单独放在matlab文件里
...
// Returns modulo inverse of a with respect to mm using extended 是完整的句子吗？
...
int inv(int a, int mm)
{
	...
	if (mm == 1)
		return 0;
	// Apply extended Euclid Algorithm 欧几里得算法
	while (a > 1)
	{
		// q is quotient
		q = a / mm;
...
//find median of vector according to absolute value of elements 根据成员的绝对值寻找矢量的中值
...
//Recover frequency using information in Recovery_Info 运用recovery-info的公式来还原频率
//try torecover frequency x satisfy: rem[i] = freq % num[i]
//line_num tells which line of information of Recovery_Info should be used here 公式位置
...
//Energetic Frequencies Identification 有效（？）频率鉴定
...
void EFI(vector<vector <recovery_info> >& Recovery_Info, map <int, int>& C, vector<vector <int> >& sample_scheme, map <int, fftw_complex*>& DFT_samples, int filter_shift, int FilterShift){
	...
	for (int j = 0; j < sample_scheme.size(); ++j) {
		...
		for (int b = 0; b < k; ++b) {
			...
			//Use Chinese Reminder Theorem 中国剩余定理 to reconstruct the frequency 
...
{	
	...
	//m is the number of set of prime numbers that we will need in sampling_scheme	质数集的个数
	int m = 0;
    //参量设置
	...
	//is one set of sample scheme //是公式吗？ 样本组合之一
	sampling_scheme_recovery_info(sample_scheme, Recovery_Info, m);
	
	//count how many samples we needed in the MSFT algorithm for one filter function  计算样本量
	int total_samples = 0;
	...
	//define fftw_plans, this will later be used to store all the fftw_plan in FFTW  （一种计算FFT的c语言的完整集合）
	...
	//for j not equal to zero, fftw_plans[i][j] corresponding to FFT with size sample_scheme[i][0]*sample_scheme[i][j]  啥意思
	...
	//if bandwidth is between 2^36 to 2^64, the number of filter function we need is 4   根据带宽选择滤波器函数
	...
	//Since MSFT is a deterministic algorithm, this means that once we fixed bandwidth N and sparsity k 带宽和稀疏度来决定算法
	//the sample_scheme can be determined immediately
	//once the sample_scheme has been determined, we can precompute the values of periodic Gaussian functions 周期高斯函数的值that will be used in the sample creating function
	//for fixed bandwidth and sparsity, the conv_G 系数G can be used repeatedly for many times
	...
	int filter_shift_current; //转换当前滤波器
	...
	//only need those uniform samples that will be used in convolution卷积
	...
	//固定的简洁算法 标准样本
	//record the error for each round of numerical test. 记录每次循环的错误
	vector<double> err_record;
	//record how long time it takes for num_test repeated numerical experiments 记录循环时间
	double duration;
	//random seed
	int seed = 123;
	
	//start different numerical test 不同的数值测试
	...
	//create random frequencies and coefficients 随机生成一些频率和系数
	...
	//create needed signal samples by using function create_sample 用函数去创建信号样本
	...
	//cout << "Size of sig_samp	le is:" << sig_sample_index << endl;
	
	sig_sample_index = 0;
	
	
	
	//R is where store the frequencies and coefficients recovered from one filter (window) function从一个窗函数滤波器中恢复出的频率及系数
	//pair<frequency, coefficient>
	vector <pair<int, complex<double>>> R;
	//pick k frequencies with largest magniture 找出来最大量级频率 from R
	//Store it in R_final
	vector <pair<int, complex<double>>> R_final;
	//convert freq and coeff into the same form of R_final 转成r-final格式
	...
	start = std::clock();
	//start repeated numerical test 重复测试
	...
	//shift filter function for filter_num times to cover the whole bandwidth  重复（）次来覆盖带宽
	for (int i = 0; i < filter_num; ++i){
		
		filter_shift_current = floor(N / double(filter_num) / 2.0) + i*FilterShift;
				
		//create needed samples 采样
		...
		//DFT_samples stores the DFT of sequence in samples   DFT_samples在样本中存储序列的离散傅里叶变换
	    map <int, fftw_complex*> DFT_samples;
	    //Use FFTW to get DFT of sequence	计算dft	
	...
	freq.clear();
	coeff.clear();
	//at the end of each round, clear the sig_sample 清空 重复
	sig_sample_index = 0;
